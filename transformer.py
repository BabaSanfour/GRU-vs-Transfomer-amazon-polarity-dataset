...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        var = (torch.var(inputs, dim=-1 )*(inputs.shape[-1]-1))/inputs.shape[-1]
        mean = torch.mean(inputs, dim=-1)
        outputs = (inputs - mean.unsqueeze(-1))/torch.sqrt(var.unsqueeze(-1) +1e-9)* self.weight +self.bias
        return outputs
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.w_q = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.w_k = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.w_v = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.w_o = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        """
        attention_weights = (queries @ keys.transpose(2, 3)) / (queries.shape[3] ** 0.5)
        if mask is not None:
            attention_weights.masked_fill_(mask.unsqueeze(1).unsqueeze(2) == 0, 0)
            attention_weights_sum = attention_weights.sum(dim=-1, keepdim=True)
            attention_weights = attention_weights / (attention_weights_sum + 1e-9)
    ...
    def split_heads(self, tensor):
        ...
        output = torch.reshape(tensor, (tensor.shape[0], tensor.shape[1], self.num_heads, int(tensor.shape[-1]//self.num_heads)))
    ...
    def merge_heads(self, tensor):
        ...
        output = torch.reshape(output, (tensor.shape[0], tensor.shape[2], int(tensor.shape[-1]*self.num_heads)))
        return output
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        attention_outputs = self.layer_norm_1(x)
        attention_outputs = self.attn(attention_outputs, mask)
        outputs = self.layer_norm_2(x + attention_outputs)
        outputs = self.linear(outputs)
        return x + attention_outputs + outputs
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask = torch.cat([ torch.ones(mask.shape[0], 1).to(mask.device), mask], dim=-1)
        x = torch.cat([cls_token, x], dim=1)
        x = x + self.pos_embedding[:,:T+1].clone()
